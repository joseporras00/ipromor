...
with open('prueba.csv') as csvfile:
    ...
    for row in reader:
        ...
        cursor.execute("SELECT fechahora from prueba WHERE fechahora=?",row[0])
        data=cursor.fetchone()

        if data is None:
            dato = [row[0],row[1],row[2],row[3],row[4],row[5]]

            cursor.execute("INSERT INTO prueba(fechahora,cliente,maquina,tipo,estado,descripcion) VALUES(?,?,?,?,?,?)",
                       dato[0], dato[1], dato[2], dato[3], dato[4], dato[5])
            con.commit()
